...
describe('Express Query Parameters', function () {
  describe('General', function () {
    ...
    it('Lean', function () {
      assert.equal(queryParameters.parse({lean: 'true'}).lean, true)
      assert.equal(queryParameters.parse({lean: true}).lean, true)
    })
    it('Skip', function () {
      assert.equal(queryParameters.parse({skip: '4'}).skip, 4)
      assert.equal(queryParameters.parse({skip: 4}).skip, 4)
    })
    it('Sort', function () {
      assert.equal(queryParameters.parse({sort: 'name'}).sort, 'name')
    })
    it('Limit', function () {
      assert.equal(queryParameters.parse({limit: '2'}).limit, 2)
      assert.equal(queryParameters.parse({limit: 2}).limit, 2)
    })
    it('Select', function () {
      assert.deepEqual(queryParameters.parse({select: 'name'}).select, { name: 1 })
      assert.deepEqual(queryParameters.parse({select: 'name title'}).select, { name: 1, title: 1 })
    })
  })
  describe('Mongoose - Parse', function () {
    it('gt', function () {
      assert.equal(queryParameters.parse({where: 'name', gt: '2016-10-20'}).gt, '2016-10-20')
    })
    it('lt', function () {
      assert.equal(queryParameters.parse({where: 'name', lt: '2016-10-20'}).lt, '2016-10-20')
    })
    it('gte', function () {
      assert.equal(queryParameters.parse({where: 'name', gte: '2016-10-20'}).gte, '2016-10-20')
    })
    it('lte', function () {
      assert.equal(queryParameters.parse({where: 'name', lte: '2016-10-20'}).lte, '2016-10-20')
    })
    it('in', function () {
      assert.equal(queryParameters.parse({where: 'roles', in: 'admin'}).in, 'admin')
    })
    it('ne', function () {
      assert.equal(queryParameters.parse({where: 'roles', ne: 'admin'}).ne, 'admin')
    })
    it('nin', function () {
      assert.equal(queryParameters.parse({where: 'roles', nin: 'admin'}).nin, 'admin')
    })
    it('regex', function () {
      assert.equal(queryParameters.parse({where: 'name', regex: 'ja'}).regex, 'ja')
    })
    it('options', function () {
      assert.equal(queryParameters.parse({where: 'name', options: '2016-10-20'}).options, '2016-10-20')
    })
    it('size', function () {
      assert.equal(queryParameters.parse({where: 'roles', size: '3'}).size, '3')
    })
    it('all', function () {
      assert.equal(queryParameters.parse({where: 'name', all: '2016-10-20'}).all, '2016-10-20')
    })
    it('or', function () {
      assert.deepEqual(queryParameters.parse({or: {roles: ['admin', 'help']}}).or, { '$or': [ { roles: 'admin' }, { roles: 'help' } ] })
    })
    it('nor', function () {
      assert.deepEqual(queryParameters.parse({nor: {roles: ['admin', 'help']}}).nor, { '$nor': [ { roles: 'admin' }, { roles: 'help' } ] })
    })
    it('and', function () {
      assert.deepEqual(queryParameters.parse({and: {roles: ['admin', 'help']}}).and, { '$and': [ { roles: 'admin' }, { roles: 'help' } ] })
    })
    it('aggregate', function () {
      assert.deepEqual(queryParameters.parse({aggregate: {roles: ['admin', 'help']}}).aggregate, [{roles: ['admin', 'help']}])
    })
    it('populateId', function () {
      assert.equal(queryParameters.parse({populateId: 'name title person'}).populateId, 'name title')
    })
    it('populateItems', function () {
      assert.equal(queryParameters.parse({populateItems: 'name role content'}).populateItems, 'name content')
    })
    it('deepPopulate', function () {
      assert.equal(queryParameters.parse({deepPopulate: 'name.test role content.type'}).deepPopulate, 'name.test role content.type')
    })
    it('where - gt & lt', function () {
      assert.deepEqual(queryParameters.parse({where: 'created', gt: '2015-11-17', lt: '2019-12-30'}).where, { created: { '$gt': '2015-11-17', '$lt': '2019-12-30' } })
    })
  })
